<?php 
 ...
 if($products->num_rows > 0){
    ...
    while($ir = $inventory->fetch_assoc()){
        $ir['price'] = is_numeric($ir['price']) ? ($ir['price'] + 0) : 0;
        ...
        <div class="row gx-4 gx-lg-5 align-items-center">
            <div class="col-md-6">
                ...
                <div class="mt-2 row gx-2 gx-lg-3 row-cols-4 row-cols-md-3 row-cols-xl-4 justify-content-start">
                    <?php 
                        if(is_array($fileO) && !empty($fileO)):
                        foreach($fileO as $k => $gallery_img):
                            if(in_array($gallery_img,array('.','..')))
                                continue;
                    ?>
                    <div class="col">
                        <a href="javascript:void(0)" class="view-image <?php echo $k == 2 ? "active":'' ?>"><img src="<?php echo validate_image('uploads/product_'.$id.'/'.$gallery_img) ?>" loading="lazy"  class="img-thumbnail" alt=""></a>
                    </div>
            ...
            <div class="col-md-6">
                ...
                <div class="fs-5 mb-5">
                <span id="price"><?php echo format_price(isset($inv[0]['price']) ? $inv[0]['price'] : 0) ?></span>
                <br>
...
<script>
    ...
    function formatPrice(n){
        n = parseFloat(n) || 0;
        return 'Ksh ' + n.toLocaleString(window.APP_LOCALE || 'en-KE');
    }
    $(function(){
        ...
        $('.variant-item').click(function(){
            ...
            $(this).addClass("active")
            if(!!inv[k]){
                $('#price').text(formatPrice(inv[k].price))
