                    ...
                    <div class="d-flex w-100 justify-content-between  mb-2 py-2 border-bottom cart-item">
                        <div class="d-flex align-items-center col-8">
                            ...
                            <div>
                                ...
                                <p class="mb-1 mb-sm-1"><small><b>Price:</b> <span class="price"><?php echo format_price($row['price']) ?></span></small></p>
                                <div>
                        ...
                        <div class="col text-right align-items-center d-flex justify-content-end">
                            <h4><b class="total-amount"><?php echo format_price($row['price'] * $row['quantity']) ?></b></h4>
                        </div>
                    </div>
                <?php endwhile; ?>
...
<script>
    function calc_total(){
        ...
        $('.total-amount').each(function(){
            amount = $(this).text()
            amount = amount.replace(/Ksh\s?/g,'').replace(/\,/g,'')
            amount = parseFloat(amount)
            total += amount
        })
        $('#grand-total').text('Ksh ' + parseFloat(total).toLocaleString(window.APP_LOCALE || 'en-KE'))
    }
    function qty_change($type,_this){
        ...
        var price = _this.closest('.cart-item').find('.price').text()
            price = price.replace(/Ksh\s?/g,'').replace(/,/g,'')
            console.log(price)
        ...
        new_total = 'Ksh ' + parseFloat(qty * price).toLocaleString(window.APP_LOCALE || 'en-KE')
